...
# ─────────────────────────────────────────────────────
#  GLOBAL CSS  (dark, professional, crypto vibe)
# ─────────────────────────────────────────────────────
st.markdown("""
<style>
/* ── Google Font ── */
...
/* ── Sliders ── */
.stSlider [data-baseweb="slider"] div[role="slider"] { background: #f59e0b; }
.stSlider [data-baseweb="slider"] div[data-testid="stTickBar"] { color: #64748b; }

/* ── DataFrames ── */
.stDataFrame { border: 1px solid #1e293b; border-radius: 8px; overflow: hidden; }
.stDataFrame thead th { background: #1e293b !important; color: #f59e0b !important; font-weight: 600; }
...
/* ── Progress bar ── */
.stProgress .st-bo { background: linear-gradient(90deg, #f59e0b, #22c55e); }
...
# ─────────────────────────────────────────────────────
@st.cache_data
def load_csv(file_obj, src="upload"):
    if src == "upload":
    ...
    for i in range(swing_len * 2, len(df)):
        ...
        if position is None:
            ...
            if exit_price is not None:
                ...
                if position == "long":
                    pnl = size * (exit_price - entry) - fee_cost
                else:
                    pnl = size * (entry - exit_price) - fee_cost
...
with st.sidebar:
    ...
    if data_source == "Upload CSV":
        uploaded = st.file_uploader("Drop your BTC CSV", type=["csv"], label_visibility="collapsed")
        if uploaded:
            df_raw = load_csv(uploaded)
            st.success(f"✓ {len(df_raw):,} candles loaded")
    else:
        st.info("Using built-in synthetic BTC data (2018–2025)")
    ...
    fee          = st.number_input("Fee per side (%)", 0.0, 0.5, 0.04, step=0.01,
                                   format="%.2f") / 100
    slippage     = st.number_input("Slippage (%)", 0.0, 0.5, 0.03, step=0.01,
                                   format="%.2f") / 100
...
if df_raw is None:
    np.random.seed(42)
    dates = pd.date_range("2018-01-01", "2025-12-31", freq="4h")
    price = 7000.0
    prices = []
    for _ in range(len(dates)):
        price *= np.exp(np.random.normal(0.0002, 0.022))
    ...
    df_raw = pd.DataFrame({
        ...
        "Close":  prices,
        "Volume": np.random.uniform(500, 80000, len(dates)),
    }, index=dates)
...
with col_d1:
    date_start = st.date_input("From", df_raw.index[0].date(), min_value=df_raw.index[0].date(), max_value=df_raw.index[-1].date())
with col_d2:
    date_end   = st.date_input("To",   df_raw.index[-1].date(), min_value=df_raw.index[0].date(), max_value=df_raw.index[-1].date())
...
with tab_overview:
    ...
    for col, label, val, sub, clr in cards:
        with col:
    ...
    with col_r:
        ...
        fig_dist.add_trace(go.Histogram(
            x=daily_returns, nbinsx=80, name="Daily Returns",
            marker=dict(color="#f59e0b", line=dict(color="#0a0e1a", width=0.5)),
            opacity=0.8,
        ))
...
with tab_chart:
    ...
    if show_swings:
        ...
        swing_highs = df_chart[sh_s]
        swing_lows  = df_chart[sl_s]
        fig.add_trace(go.Scatter(
            x=swing_highs.index, y=swing_highs["High"] * 1.002,
            mode="markers", marker=dict(symbol="triangle-down", color="#ef4444", size=8),
            name="Swing High",
        ), row=1, col=1)
        fig.add_trace(go.Scatter(
            x=swing_lows.index, y=swing_lows["Low"] * 0.998,
    ...
    if run_btn or True:  # auto-run on load
        ...
        if result is None:
            ...
            cards_bt = [
                ...
                (c6, "Profit Factor", f"{(result['WinRate']/100*result['RR']) / (1-result['WinRate']/100):.2f}" if result['WinRate'] < 100 else "∞", "W×RR / L", "positive"),
            ]
            for col, label, val, sub, clr in cards_bt:
                with col:
                    st.markdown(metric_card(label, val, sub, clr), unsafe_allow_html=True)
            ...
            fig_eq.update_layout(**PLOTLY_LAYOUT, height=500,
            ...
            if result["trade_records"]:
                ...
                with col_p1:
                    ...
                    fig_pnl.add_vline(x=0,       line_dash="dash", line_color="#64748b", line_width=1)
                ...
                with col_p2:
                    ...
                    fig_cum.add_trace(go.Scatter(
                        ...
                        line=dict(color="#818cf8", width=2), name="Cumulative PnL",
                        fill="tozeroy",
                ...
                with col_t2:
                    fig_type = go.Figure(data=[
                        go.Bar(name="Total PnL ($)", x=by_type["type"], y=by_type["Total_PnL"],
                               marker_color=["#22c55e" if v > 0 else "#ef4444" for v in by_type["Total_PnL"]]),
                    ])
                    fig_type.update_layout(**PLOTLY_LAYOUT, height=220, xaxis_title="Trade Type")
...
with tab_optimizer:
    ...
    if not risk_options or not rr_options:
        ...
        <div class="info-box">
            ...
            Risk {risk_options} · RR {rr_options}
        </div>""", unsafe_allow_html=True)
            ...
            for sw in range(swing_range[0], swing_range[1] + 1):
                for rk in risk_options:
                    for rv in rr_options:
                        ...
                        status.markdown(f"Testing Swing={sw} · Risk={rk}% · RR={rv}… ({done}/{total})")

            progress.empty()
            status.empty()

            if opt_results:
                df_opt = pd.DataFrame(opt_results).sort_values("Return%", ascending=False)
        ...
        if "opt_results" in st.session_state:
            ...
            # Scatter plot
            section_header("🔬", "Return vs Drawdown (Risk-Adjusted View)")
            fig_scatter = px.scatter(
                ...
                labels={"MaxDD%": "Max Drawdown (%)", "Return%": "Total Return (%)"},
            )
            ...
            # Heatmap: Swing × RR (best risk for each)
            section_header("🌡️", "Return Heatmap — Swing Period × RR")
            best_per = df_opt.groupby(["Swing", "RR"])["Return%"].max().reset_index()
            pivot_h  = best_per.pivot(index="Swing", columns="RR", values="Return%")

            fig_hm = go.Figure(go.Heatmap(
                z=pivot_h.values, x=[f"RR={v}" for v in pivot_h.columns],
            ...
            # Download
            csv_bytes = df_opt.to_csv(index=False).encode()
            st.download_button("⬇️  Download All Results (CSV)", csv_bytes,
    ...
    if "last_result" not in st.session_state:
        ...
        if len(R_list) < 5:
            ...
            with col_mc2:
                st.markdown("""
                <div class="info-box">
                    Monte Carlo resamples the historical trade R-multiples thousands of times in
                    random order to estimate the distribution of possible outcomes — stress-testing
                    your strategy against sequence-of-returns risk.
                </div>""", unsafe_allow_html=True)
            ...
            if mc_btn or "mc_curves" in st.session_state:
                ...
                mc_cards = [
                    ...
                    (c6, "Loss Prob.",    f"{prob_loss:.1f}%", "Below initial capital", "negative" if prob_loss > 20 else "positive"),
                ]
                for col, label, val, sub, clr in mc_cards:
                    with col:
                        st.markdown(metric_card(label, val, sub, clr), unsafe_allow_html=True)
                ...
                with col_mca:
                    ...
                    fig_mch.add_trace(go.Histogram(
                        x=curves, nbinsx=60,
                        marker=dict(color=[
                            "#ef4444" if c < initial_capital else "#22c55e" for c in curves
                ...
                with col_mcb:
                    ...
                    fig_sor.add_trace(go.Scatter(
                        ...
                        line=dict(color="#818cf8", width=2), name="Sorted Outcomes",
                        fill="tozeroy",
...
with tab_trades:
    ...
    if "last_result" not in st.session_state or not st.session_state["last_result"].get("trade_records"):
        ...
        trades_df["pnl_color"] = trades_df["pnl_currency"].apply(lambda x: "🟢" if x > 0 else "🔴")
        trades_df["Result"]    = trades_df["pnl_currency"].apply(lambda x: "WIN" if x > 0 else "LOSS")

        # Filters
        col_f1, col_f2, col_f3 = st.columns(3)
        with col_f1:
            type_filter = st.multiselect("Trade Type", ["long", "short"], default=["long", "short"])
        with col_f2:
            result_filter = st.multiselect("Result", ["WIN", "LOSS"], default=["WIN", "LOSS"])
        with col_f3:
            sort_by = st.selectbox("Sort by", ["entry_time", "pnl_currency", "R"], index=0)
        ...
        ft_cards = [
            ...
            (c4, "Avg R",           f"{filtered['R'].mean():.3f}", "average R multiple", "positive" if filtered['R'].mean()>0 else "negative"),
        ]
        for col, label, val, sub, clr in ft_cards:
            with col:
                st.markdown(metric_card(label, val, sub, clr), unsafe_allow_html=True)
        ...
        # Display table
        display_cols = ["pnl_color", "type", "entry_time", "entry_price", "exit_time",
                        "exit_price", "pnl_currency", "R", "size", "risk_amount"]
        ...
        st.dataframe(
            filtered[available_cols].rename(columns={
                ...
                "Entry $": "${:.2f}", "Exit $": "${:.2f}",
                "PnL ($)": "${:.2f}", "R Multiple": "{:.3f}",
                "Position Size": "{:.4f}", "Risk ($)": "${:.2f}",
            }).apply(lambda row: [
                "background-color: rgba(34,197,94,0.08)" if row["PnL ($)"] > 0
                else "background-color: rgba(239,68,68,0.08)"
            ] * len(row), axis=1),
            use_container_width=True, height=460,
        )

        # Download
        csv_trades = filtered.drop(columns=["pnl_color"]).to_csv(index=False).encode()
        st.download_button("⬇️  Export Trade Log (CSV)", csv_trades,
                           "trade_log.csv", "text/csv", use_container_width=True)

        # PnL timeline scatter
        ...
        fig_tl.add_trace(go.Scatter(
            ...
            text=filtered.apply(lambda r: f"{r['type'].upper()}<br>PnL: ${r['pnl_currency']:.2f}<br>R: {r['R']:.3f}", axis=1),
...
# ── Footer ──
st.markdown("""
<div style="text-align:center;padding:32px 0 16px;color:#334155;font-size:12px;border-top:1px solid #1e293b;margin-top:32px">
